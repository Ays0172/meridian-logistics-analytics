...
A worked custom function, since you'll want exactly this reusable across every `dd/MM/yyyy`-style text date you meet this week (not just `FactTarget`'s):
...
### 8. Data type discipline and locale — landmine #7, the silent one

`FactTarget`'s mirror CSV stores `TargetMonthDateKey` as **text**, in `dd/MM/yyyy` format. This is the most dangerous landmine in the whole contract, precisely because it does not look dangerous: apply Power Query's default "Change Type → Date" to a `dd/MM/yyyy` text column on a machine whose regional settings assume `M/d/yyyy` (US-style), and for any day-of-month **12 or below**, the conversion succeeds — silently, with no error — and swaps day and month. `15/03/2025` fails loudly (there's no 15th month, so you get an `[Error]` you'd actually notice). `03/07/2025` succeeds *silently* as "March 7th" when the source data meant "3rd of July" — no error, no warning, a wrong date sitting in your model looking completely plausible, and every measure that filters or groups by that date now quietly wrong for however many rows fall into the ambiguous zone.
...
| 10 | Leading-zero business keys in `DimSku.SkuCode`'s mirror CSV | Import explicitly as text. Power Query's automatic type detection will happily read `SKU-000001`-style codes fine since they contain letters, but any purely numeric leading-zero business key (watch for this pattern generally) will silently lose its leading zeros the instant it's auto-detected as a number. |
...
**5. Walk into landmine #7, then fix it (10 min).** Load `FactTarget`'s mirror CSV with Power Query's default date type conversion (whatever your machine's locale gives you) and record the interpreted date for two specific source rows: one with day > 12, and one with day ≤ 12. Then reload using the explicit `"en-GB"` locale conversion and record the same two dates again. Done = both interpretations recorded for both rows, and a one-sentence statement of which row silently gave you a wrong answer under the default conversion.
...
- [ ] Landmine #7 walked into (wrong date recorded for the ambiguous row under default locale) and then correctly fixed with an explicit `"en-GB"` conversion.
