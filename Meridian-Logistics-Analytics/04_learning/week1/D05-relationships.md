...
One more mechanical detail worth stating precisely, because it's exactly where the `-1` Unknown-member convention from Day 3 earns its keep here: a shipment still `In Transit` has not yet had its `AtaDateKey` or `DeliveryDateKey` event occur. Per the contract's global convention (§0 — "every dimension has key `-1`... facts use `-1` rather than null for FKs"), that unresolved date key resolves to `DimDate`'s own `-1`/Unknown row, the same as any other unresolved FK in this model. This is precisely why the convention is universal rather than date-specific: if `AtaDateKey` were left null instead, that in-transit shipment would simply vanish from any visual filtered through the `AtaDateKey` role-play relationship — not show up as "unknown," just disappear — which is the exact same silent-undercount failure mode Day 3 warned you about, now happening specifically inside a role-playing relationship instead of an ordinary one.
