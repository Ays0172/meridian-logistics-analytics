...
| 3 | Deadhead % vs. Truck Utilisation, by carrier | `Deadhead %`, `Truck Utilisation %` | 100%-stacked bar, by carrier | Reconciliation check (do the two sum near 100%, per the KPI dictionary's own watch-out) and a fleet-efficiency comparison in one visual. |
...
silently forced to reconcile. A 100%-stacked bar makes that reconciliation visible
by construction: if a carrier's stack doesn't reach the top of the chart area (with
gridlines showing), that gap *is* the unaccounted distance, sitting in the visual
itself rather than in a footnote nobody reads. This is the same instinct as Day
23's scatter plot: pick a chart type that makes the trap visible rather than one
that requires trusting a number in isolation.

### Carrier Composite Score, and why it needs a drillthrough

`LND.CAR.SCORE` is a min-max normalised weighted index
(`0.40 x NormOnTime + 0.25 x NormFirstAttempt + 0.20 x (1 - NormCostIndex) +
0.15 x (1 - NormSubcontractDiscipline)`), and the KPI dictionary's own watch-out is
...
measures from Exercise 24.3) to `_Measures`, display folder `03 Landside`.
